...
class MIA(EvalMetric):
    ...
    def SVC_fit_predict(shadow_train, shadow_test, target_train, target_test):
        ...
        Y_shadow = np.concatenate([np.ones(n_shadow_train), np.zeros(n_shadow_test)]).to(device)#1,1,1,0,0,0
    ...
    def SVC_MIA(self, train_pos, train_neg, test_pos, test_neg, model, device):
        ...
        train_pos_labels.to(device)
        train_neg_prob.to(device)
        train_neg_labels.to(device)
        test_pos_prob.to(device)
        test_pos_labels.to(device)
        test_neg_prob.to(device)
        test_neg_labels.to(device)
        ...
        train_pos_data.to(device)
        train_neg_data.to(device)
        test_pos_data.to(device)
        test_neg_data.to(device)
