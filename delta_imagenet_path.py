...
def prepare_data(
    ...
    if dataset == "imagenet10":
        ...
        ###with path
        train_set = ImageFolderWithPaths(os.path.join(data_path, 'train'), transform=train_transform)
        train_set_for_test = ImageFolderWithPaths(os.path.join(data_path, 'train'), transform=validation_transform)
        #val set 2
        val_set = ImageFolderWithPaths(os.path.join(data_path, 'validation'), transform=validation_transform)
        ...
        if adv is not None:
            print("adv",adv,"&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
            adv_path = adv
            train_set_adv = ImageFolderWithPaths(os.path.join(adv_path,"train_same_folder"), transform=validation_transform)
            val_set_adv = ImageFolderWithPaths(os.path.join(adv_path,"validation_same_folder"), transform=validation_transform)
        elif cor is not None:
            print("cor",cor,"&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
            adv_path = cor
            train_set_adv = ImageFolderWithPaths(os.path.join(adv_path,"train",cor_type,level), transform=validation_transform)
            val_set_adv = ImageFolderWithPaths(os.path.join(adv_path,"validation",cor_type,level), transform=validation_transform)
        else:
            print("Wrong")
