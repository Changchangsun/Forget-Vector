...
def RL(data_loaders, model, criterion, optimizer, epoch, args, mask=None):
    ...
    if args.dataset == "cifar100" or args.dataset == "TinyImagenet" :
        ...
        ##new train dataset for fine-tuning
    ...
    elif args.dataset == "cifar10" or args.dataset == "svhn" or args.dataset == "imagenet10"or args.dataset == "cifar10_scc":
        print("RL train phase,",args.dataset)
        losses = utils.AverageMeter()
        top1 = utils.AverageMeter()
        ...
        if epoch < args.warmup: 
            print("### if epoch < args.warmup:")
            utils.warmup_lr(epoch, i+1, optimizer,
                            one_epoch_step=loader_len, args=args)
        
        for i, (image, target) in enumerate(forget_loader):
            image = image.cuda()
            target = torch.randint(0, args.num_classes, target.shape).cuda()# new random label data
            output_clean = model(image)
            loss = criterion(output_clean, target)
