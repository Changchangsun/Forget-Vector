...
def cifar10_dataloaders(
    ...
    print("10000 images for testing\t no normalize applied in data_transform")
    print("Data augmentation = randomcrop(32,4) + randomhorizontalflip")
    print("Data was downloaded to ",data_dir)
    ...
    ###start: not work for "train_full_loader, val_loader, _ " ###
    """
    class_to_replace=args.class_to_replace, help="Specific class to forget", type=int,default=0
    num_indexes_to_replace=args.num_indexes_to_replace, help="Number of data to forget",type=int,default=None,
    #0
    #1
    
    indexes_to_replace=args.indexes_to_replace, help="Specific index data to forget",type=list, default=None,
    """
    if class_to_replace is not None and indexes_to_replace is not None:
        raise ValueError(
            "Only one of `class_to_replace` and `indexes_to_replace` can be specified"
    ...
    if class_to_replace is not None:
        print("###################### class_to_replace is not None")
        replace_class(
            train_set,
            class_to_replace,
            num_indexes_to_replace=num_indexes_to_replace,
            seed=seed - 1,
            only_mark=only_mark, #True
        )
        if num_indexes_to_replace is None or num_indexes_to_replace == 4500:
            #remove the forget class from the test set
            test_set.data = test_set.data[test_set.targets != class_to_replace]
            test_set.targets = test_set.targets[test_set.targets != class_to_replace]
    if indexes_to_replace is not None:
        print("################## indexes_to_replace is not None")
        replace_indexes(
            dataset=train_set,
            indexes=indexes_to_replace,
            seed=seed - 1,
    ...
    print("train_set",len(train_set))#train_set 45000
    print("val_loader",len(valid_set))#val_loader 20
    print("test_loader",len(test_set))#test_loader 36
# train_set 45000
# val_loader 5000
# test_loader 10000
